...
;(function($, window, document) {
	...
	var getRepos = function(user, opts, callback) {
		...
		success: function(results) {
                    ...
                    if (results.data.name !== undefined) {
                        element.empty().append('<a href="http://3rd-ndn-hackathon.named-data.net/' + repo + '" target="_blank"><i class="fa fa-file-text-o"></i></a>');
                    }
		}
	    });
        };
	...
	var putRepos = function(elm, args) {
	    ...
	    var n = args.length-1;
            $.each(
                $.grep(args, function(repo, i) { return (repo.name !== "3rd-ndn-hackathon.github.io"); }),
                function(i, repo) {
...
$(function () {
    $('#github-repo').showGitHubRepo('3rd-ndn-hackathon', {
	count: 10
    });
});
